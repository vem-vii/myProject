<template>
  <div class="app-container">
    ...
    <div style="margin-top: 15px">
      <el-input
        placeholder="请输入需要添加的分类,左边下拉选择分类等级"
        v-model="input"
...
export default {
  ...
  methods: {
    ...
    // 编辑文章分类
    editBlogTypeHandle({ _id }) {
      findOneBlogType(_id).then((res) => {
        this.form = res.data;
    ...
    addBlogTypeHandle() {
      if (this.input) {
        ...
        this.$message.error("输入分类不能为空");
      }
    },
    // 删除文章分类
    deleteBlogTypeHandle({ _id }) {
